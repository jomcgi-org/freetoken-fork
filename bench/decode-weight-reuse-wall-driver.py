"""Native decode weight reuse off/on at fixed client and server concurrency four."""
import hashlib, importlib.util, json, os, pathlib, re, shlex, signal, subprocess, sys, time, urllib.error, urllib.request
from concurrent.futures import ThreadPoolExecutor, as_completed
R = pathlib.Path('/var/lib/longhorn/nvme-02/freetoken')
WT = R / 'wt-astra-decode-weight-reuse'
SRC = R / 'wt-plegather'
OUT = R / 'results'
...
def wait_gpu_release():
    ...
    while True:
        result = run('nvidia-smi', '--query-compute-apps=pid', '--format=csv,noheader,nounits', capture_output=True, timeout=10)
        if not result.stdout.strip():
            return
        if time.monotonic() > deadline:
            raise RuntimeError('GPU processes remain after service shutdown: ' + result.stdout.strip())
        time.sleep(1)
...
for flag, value in [('--moe-disk-prefill', 'staged'), ('--port', '18090'), ('--moe-hot-adapt-interval-steps', 'auto'), ('--kv-disk-cache-gib', '0'), ('--max-running-requests', '4')]:
    args[args.index(flag) + 1] = value
args.remove('--moe-collect-stats')
args.extend(['--moe-hot-plan-persist', 'off', '--cache-type', 'naive', '--moe-disk-prefill-io', 'buffered', '--kv-ladder', 'off', '--kv-reserve-tokens', '65536', '--cuda-graph-max-bs', '4'])
assert '--moe-collect-stats' not in args
assert '--moe-step-timing' not in args
...
assert revisions['baseline'].startswith('b8ac3f7'), revisions
assert revisions['baseline'] == revisions['optimized'], revisions
assert revisions['optimized'].startswith('b8ac3f7'), revisions
...
base_report = dict(
    ...
    policies={
        'baseline': 'Native decode weight reuse off; four active client requests on the fixed four-request server at b8ac3f7.',
        'optimized': 'Native decode weight reuse on; four active client requests on the identical server and native binary at b8ac3f7.',
    },
)

validation_path = OUT / 'astra-decode-weight-reuse-v2-validation.json'
validation = json.loads(validation_path.read_text())
assert validation['completed'] and validation['checks_passed']
assert validation['targeted_cases_passed_without_skips'] == 44
assert validation['revision'] == revisions['baseline']
assert validation['original_serving_restoration']['choices'][0]['message']['content'].strip() == 'OK'
base_report['native_validation'] = dict(revision=validation['revision'], native_sha256=validation['native_sha256'], record_sha256=hashlib.sha256(validation_path.read_bytes()).hexdigest())
native_extensions = {}
for mode, tree in [('baseline', WT), ('optimized', WT)]:
    ...
    assert b'set_decode_weight_reuse' in data, binary
...
assert native_extensions['baseline']['sha256'] == validation['native_sha256']
base_report['native_extensions'] = native_extensions

report = dict(base_report)
report_path = OUT / 'astra-decode-weight-reuse-wall-driver-metadata.json'
...
assert not list(OUT.glob('astra-decode-weight-reuse-wall-r[1-4]*'))
...
for other in ('astra-concurrent-wall-driver', 'astra-decode-weight-reuse-validation-v2', 'astra-sustained-hot-staging-wall-driver', 'astra-sustained-reader-wall-driver', 'astra-hot-staging-validation', 'astra-sustained-cached-wall-driver'):
...
try:
    ...
    for label, mode in [('r1', 0), ('r2', 1), ('r3', 1), ('r4', 0)]:
        ...
        file_io = 'mmap'
        concurrency = 4
        env['FREETOKEN_CPU_MOE_DECODE_WEIGHT_REUSE'] = str(mode)
        ...
        report = dict(base_report, args=start_args, env=dict(env), revision=revision, source_tree=str(tree), label=label, mode=policy, file_io=file_io, client_concurrency=concurrency, decode_weight_reuse=bool(mode), records=[], phase='startup')
        report_path = OUT / f'astra-decode-weight-reuse-wall-{label}-metadata.json'
        stop()
        wait_gpu_release()
        ...
        try:
            ...
            setting = next(value.split(b'=', 1)[1].decode() for value in pathlib.Path(f'/proc/{worker}/environ').read_bytes().split(b'\0') if value.startswith(b'FREETOKEN_CPU_MOE_DECODE_WEIGHT_REUSE='))
            assert setting == str(mode), setting
            report['worker_decode_weight_reuse_env'] = setting
            ...
            report['capacity_startup'] = [line for line in journal.splitlines() if 'CPU MoE executor ready:' in line or 'Start capturing CUDA graphs with sizes:' in line]
            assert any('CPU MoE executor ready:' in line and 'max_tokens=4' in line and 'avx512vnni(nvfp4-w4a8)' in line for line in report['capacity_startup']), report['capacity_startup']
            graph_sizes = [json.loads(match.group(1)) for line in report['capacity_startup'] if (match := re.search(r'Start capturing CUDA graphs with sizes: (\[[0-9, ]+\])', line))]
            assert len(graph_sizes)==1 and 4 in graph_sizes[0], report['capacity_startup']
            ...
            report['phase'] = 'timing'
            save()
            print('READY '+label+' concurrency='+str(concurrency)+' decode_weight_reuse='+str(bool(mode)), flush=True)
            client_result = subprocess.run([str(SRC/'.venv/bin/python'), str(WT/'bench/concurrent-prefill-wall.py'), '--tokenizer', str(R/'models/flash-e2m1.ftw'), '--output', str(OUT/f'astra-decode-weight-reuse-wall-{label}.jsonl'), '--mode', policy, '--warmup-pairs', '2', '--blocks', '6', '--io-pid', str(worker), '--concurrency', str(concurrency)], env=baseenv, timeout=2400, check=False)
            report['client_returncode'] = client_result.returncode
            rows = [json.loads(line) for line in (OUT/f'astra-decode-weight-reuse-wall-{label}.jsonl').read_text().splitlines()]
            assert len(rows) == 16, len(rows)
            assert all(row['diagnostic_phase_io'] is False for row in rows), 'diagnostic rows cannot qualify wall time'
            report['client_manifest'] = json.loads((OUT/f'astra-decode-weight-reuse-wall-{label}.prompts.json').read_text())
            report['client_summary'] = json.loads((OUT/f'astra-decode-weight-reuse-wall-{label}.summary.json').read_text())
            assert [g['phase'] for g in report['client_summary']] == ['warmup', 'measured']
            assert [g['requests'] for g in report['client_summary']] == [4, 12]
            assert sorted(row['ordinal'] for row in rows) == list(range(16))
            assert all(row['client_concurrency'] == concurrency for row in rows)
            assert all(g['concurrency'] == concurrency for g in report['client_summary'])
            # A complete client record with failed output checks is evidence,
            # not a reason to drop the opposite policy or silently retry it.
            report['main_response_checks_passed'] = all(g['all_response_checks_passed'] for g in report['client_summary'])
            report['phase'] = 'fidelity'
            save()
            essay = next(row for row in report['client_manifest'] if row['kind'] == 'essay')
            ...
            for name, question, expected in fidelity.cases():
                ...
                fidelity_inputs.append(dict(kind='fidelity_'+name, prompt=prompt, expected=expected))
            report['fidelity_manifest'] = fidelity_inputs
            save()

            def score_fidelity(case):
                row = dict(case, mode=policy, client_concurrency=concurrency)
                try:
                    response = completion(18090, case['prompt'], 64)
                    text = response['choices'][0]['message']['content']
                    assert response['usage']['prompt_tokens'] >= 1024, response
                    reason = response['choices'][0]['finish_reason']
                    row.update(text=text, passed=text.strip()==case['expected'] and reason=='stop', usage=response['usage'], finish_reason=reason)
                except Exception as error:
                    row.update(passed=False, request_error=f'{type(error).__name__}: {error}')
                return row

            with ThreadPoolExecutor(max_workers=concurrency) as pool:
                futures = [pool.submit(score_fidelity, case) for case in fidelity_inputs]
