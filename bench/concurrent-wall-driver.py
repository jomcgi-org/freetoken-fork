"""One versus four offered requests on a fixed four-request server."""
...
WT = R / 'wt-astra-concurrent-wall-client'
SRC = R / 'wt-plegather'
OUT = R / 'results'
UNIT = 'astra-concurrent-wall-server'
...
assert revisions['baseline'].startswith('c0775ea'), revisions
assert revisions['baseline'] == revisions['optimized'], revisions
assert revisions['optimized'].startswith('c0775ea'), revisions
...
base_report = dict(
    ...
    design='Four isolated starts offer client concurrency 1, 4, 4, 1. All use the same frozen runtime, server capacity four, mmap HOT staging, buffered staged GPU DISK prefill, and a fixed 65536-token KV reserve with the single-request-only ladder disabled. Only client concurrency changes. Each source tree stays fixed throughout startup, four warmup responses, twelve measured complete-response tasks, and eight fidelity cases. HOT assignments and routing histories persist within each process. Host page cache is retained between starts. Concurrent response windows can overlap, so subgroup latency is descriptive and cannot be summed into workload elapsed time; all prompts are prepared before warmup. Diagnostics, GPU timing, KV reuse, and HOT persistence are off.',
    policies={
        'baseline': 'One active client request on the fixed four-request server at c0775ea.',
        'optimized': 'Four active client requests on the identical four-request server at c0775ea.',
    },
)
...
report_path = OUT / 'astra-concurrent-wall-driver-metadata.json'
...
assert not list(OUT.glob('astra-concurrent-wall-r[1-4]*'))
...
for other in ('astra-sustained-hot-staging-wall-driver', 'astra-sustained-reader-wall-driver', 'astra-hot-staging-validation', 'astra-sustained-cached-wall-driver'):
...
try:
    ...
    for label, mode in [('r1', 0), ('r2', 1), ('r3', 1), ('r4', 0)]:
        ...
        report = dict(base_report, args=start_args, env=dict(env), revision=revision, source_tree=str(tree), label=label, mode=policy, file_io=file_io, client_concurrency=concurrency, records=[], phase='startup')
        report_path = OUT / f'astra-concurrent-wall-{label}-metadata.json'
        stop()
        ...
        try:
            ...
            assert any('CPU MoE executor ready:' in line and 'max_tokens=4' in line for line in report['capacity_startup']), report['capacity_startup']
            ...
            report['phase'] = 'timing'
            save()
            print('READY '+label+' concurrency='+str(concurrency), flush=True)
            client_result = subprocess.run([str(SRC/'.venv/bin/python'), str(WT/'bench/concurrent-prefill-wall.py'), '--tokenizer', str(R/'models/flash-e2m1.ftw'), '--output', str(OUT/f'astra-concurrent-wall-{label}.jsonl'), '--mode', policy, '--warmup-pairs', '2', '--blocks', '6', '--io-pid', str(worker), '--concurrency', str(concurrency)], env=baseenv, timeout=2400, check=False)
            report['client_returncode'] = client_result.returncode
            rows = [json.loads(line) for line in (OUT/f'astra-concurrent-wall-{label}.jsonl').read_text().splitlines()]
            assert len(rows) == 16, len(rows)
            assert all(row['diagnostic_phase_io'] is False for row in rows), 'diagnostic rows cannot qualify wall time'
            report['client_manifest'] = json.loads((OUT/f'astra-concurrent-wall-{label}.prompts.json').read_text())
            report['client_summary'] = json.loads((OUT/f'astra-concurrent-wall-{label}.summary.json').read_text())
            ...
            stop()
            with (OUT/f'astra-concurrent-wall-{label}-journal.log').open('w') as log:
