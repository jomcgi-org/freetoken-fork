"""Sustained buffered versus cached staging on one frozen runtime."""
import hashlib, importlib.util, json, os, pathlib, shlex, signal, subprocess, time, urllib.error, urllib.request
R = pathlib.Path('/var/lib/longhorn/nvme-02/freetoken')
WT = R / 'wt-astra-direct-io'
SRC = R / 'wt-plegather'
OUT = R / 'results'
UNIT = 'astra-sustained-reader-wall-server'
...
args.extend(['--moe-hot-plan-persist', 'off', '--cache-type', 'naive'])
...
assert revisions['baseline'].startswith('132b629'), revisions
assert revisions['baseline'] == revisions['optimized'], revisions
assert revisions['optimized'].startswith('132b629'), revisions
...
base_report = dict(
    ...
    design='Four isolated starts: buffered, cached, cached, buffered. All use the same frozen optimized runtime and staged GPU prefill. Only --moe-disk-prefill-io changes. Each source tree stays fixed throughout startup, four warmup responses, twelve measured complete-response tasks, and eight fidelity cases. HOT assignments and routing histories persist within each process. Host page cache is retained between starts. Six measured JSON/prose blocks expose early-to-late behavior; all prompts are prepared before warmup. Diagnostics, GPU timing, KV reuse, and HOT persistence are off.',
    policies={
        'baseline': 'Buffered GPU staging on optimized runtime 132b629.',
        'optimized': 'Cache-aware GPU staging on the identical optimized runtime 132b629.',
    },
)
...
report_path = OUT / 'astra-sustained-reader-wall-driver-metadata.json'
...
assert not list(OUT.glob('astra-sustained-reader-wall-r[1-4]*'))
...
try:
    ...
    for label, mode in [('r1', 0), ('r2', 1), ('r3', 1), ('r4', 0)]:
        ...
        start_args.extend(['--moe-disk-prefill-io', file_io])
        ...
        report_path = OUT / f'astra-sustained-reader-wall-{label}-metadata.json'
        stop()
        ...
        try:
            ...
            assert any('ring=64 MiB' in line and 'minimum_chunk=1024 tokens' in line and 'file_io='+file_io in line for line in report['transport_startup']), report['transport_startup']
            ...
            run(str(SRC/'.venv/bin/python'), str(WT/'bench/sustained-prefill-wall.py'), '--tokenizer', str(R/'models/flash-e2m1.ftw'), '--output', str(OUT/f'astra-sustained-reader-wall-{label}.jsonl'), '--mode', policy, '--warmup-pairs', '2', '--blocks', '6', '--io-pid', str(worker), env=baseenv, timeout=2400)
            rows = [json.loads(line) for line in (OUT/f'astra-sustained-reader-wall-{label}.jsonl').read_text().splitlines()]
            assert len(rows) == 16, len(rows)
            report['client_manifest'] = json.loads((OUT/f'astra-sustained-reader-wall-{label}.prompts.json').read_text())
            ...
            stop()
            with (OUT/f'astra-sustained-reader-wall-{label}-journal.log').open('w') as log:
