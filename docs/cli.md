...
| `--moe-step-timing` | off | Append diagnostic decode timing fields. `cpu_wake_us`: average per-layer latency from submit entry to the first worker. In the default `before` mode, `cpu_groups_us`, `cpu_gil_us`, `cpu_precb_us`, and `cpu_notify_us` decompose and sum to `cpu_wake_us`. `cpu_coord_us`: coordinator overhead around submit/sync. `cpu_d2h_us` and `cpu_h2d_us`: per-step GPU event totals for input copies and the output copy plus hot-partial add. `cpu_compute_us`: per-layer compute time. `cpu_signal_us`: per-layer signal time. `cpu_layers_per_step`: decode layer count. `cpu_expert_bytes_per_step`: expert weight bytes accessed. |
