...
| `--moe-cpu-empty-skip {off,on}` | off | With `on`, skip the coordinator callback, worker notification, and barriers for an all-hot DISK decode layer, while returning a zero CPU partial. The status line always reports `all-hot layers/step`. The maximum saving is about the empty-task handshake cost, roughly 100 us, times that count, so inspect the stat before enabling the flag. |
...
| `--moe-hot-expert-budget-gib` | 0 | Protected GPU row budget for HOT experts in DISK layers; 0 disables HOT residency |
| `--moe-hot-capacity-policy {equal,coverage}` | equal | Fix equal per-layer HOT capacity, or allocate the same total rows by marginal route coverage at startup using persisted counters first and the static profile second |
| `--moe-hot-capacity-floor` | 8 | Minimum rows per DISK layer for coverage allocation when the total row budget permits; must be at least 1 |
