...
| `--moe-cpu-executor-mode` | sleep | CPU worker synchronization: `sleep`, `spin`, or topology-aware `auto`; see the [spin placement rules](#moe-cpu-spin-placement) |
| `--moe-cpu-spin-idle-us` | 500 | Maximum time, from 0 to 1000000 microseconds, that each spinning worker polls for another task before returning to the condition-variable wait |
...
<a id="moe-cpu-spin-placement"></a>
`--moe-cpu-executor-mode` defaults to `sleep`, which keeps the condition-variable worker path. Both `spin` and `auto` require a single-socket x86 system with at least three and at most 32 physical cores, plus complete CPU and SMT placement information. If a guard fails, the executor logs the reason and uses sleep mode. Each spinning worker busy-polls for at most `--moe-cpu-spin-idle-us` after its last task and then returns to the condition-variable wait. Zero makes every worker sleep between tasks. The submit path still notifies sleeping workers. The main-thread completion poll retains its separate bounded window, and its condition-variable fallbacks are counted independently.

[Spin placement](#moe-cpu-spin-placement) uses at most physical cores minus two spinning workers. It excludes the logical CPU running the main thread when the pool is constructed and every SMT sibling on that physical core. The pool still creates the full `--moe-cpu-threads` count; workers beyond the spin cap sleep between tasks. Startup logs show the exclusions and the complete worker-to-CPU map. With `--moe-step-timing`, the status line reports main-thread `spin_fallbacks`, `worker_spin_fallbacks`, and `spin_threads` as an active-to-total ratio.
